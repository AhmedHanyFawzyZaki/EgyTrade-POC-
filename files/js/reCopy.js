    ...
    $.fn.relCopy = function (options) {
        ...
        this.each(function () {
            ...
            $(this).click(function () {
                ...
                var maxID = counter;
                if (settings.highestID != "") {
                    var oldHighestID = $('#' + settings.highestID).val();
                    maxID = parseInt(oldHighestID) + 1;
                    $('#' + settings.highestID).val(maxID);
                }
                ...
                var clone = $(master).clone(true).addClass(settings.copyClass + maxID).append(settings.append);
                ...
                // Increment Clone IDs
                if ($(clone).attr('id')) {
                    var newid = $(clone).attr('id') + (maxID + 1);
                    $(clone).attr('id', newid);
                }
                ;
                ...
                $(clone).find('[id]').each(function () {
                    var newid = $(this).attr('id') + (maxID + 1);
                    $(this).attr('id', newid);
                });
